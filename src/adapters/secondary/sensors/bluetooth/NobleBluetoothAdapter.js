...
class NobleBluetoothAdapter extends ISensorService {
  ...
  async initialize() {
    ...
    try {
      ...
      console.log('[NobleAdapter] ✓ Noble initialisé avec succès');
      return true;
  ...
  async resetNoble() {
    ...
    try {
      ...
      console.log('[NobleAdapter] ✓ Noble resetté');
    } catch (error) {
  ...
  async startScanning() {
    try {
      ...
      console.log('[NobleAdapter] ✓ Scan actif');
  ...
  async stopScanning() {
    ...
    try {
      this.noble.stopScanning();
      console.log('[NobleAdapter] ✓ Scan arrêté');
    } catch (error) {
  ...
  async connectSensor(peripheral, onDisconnect = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      peripheral.connect((error) => {
        ...
        console.log('[NobleAdapter] ✓ Connexion réussie:', address);
  ...
  async disconnectSensor(address) {
    ...
    try {
      ...
      console.log('[NobleAdapter] ✓ Déconnecté:', normalizedAddress);
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      peripheral.discoverAllServicesAndCharacteristics((error, services, characteristics) => {
        ...
        if (charCount < this.config.minCharacteristics) {
          console.warn(`[NobleAdapter] ⚠️ Seulement ${charCount}/${this.config.minCharacteristics} caractéristiques`);
          ...
          characteristics.forEach((characteristic, index) => {
            ...
            characteristic.notify(true, (error) => {
              ...
              if (notificationsSetup === targetNotifications) {
                console.log(`[NobleAdapter] ✓ Toutes les notifications actives pour ${address}`);
                resolve({
      ...
      return new Promise((resolve, reject) => {
        characteristics[0].write(command, true, (error) => {
          if (error) {
            ...
            console.log('[NobleAdapter] ✓ Commande envoyée à', address);
            resolve();
          }
        });
      });
  ...
  async cleanup() {
    ...
    try {
      ...
      console.log('[NobleAdapter] ✓ Nettoyage terminé');
